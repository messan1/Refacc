...
const SpeakerBox = ({right,left,img,content,titre,person}) => {
  return (
    ...
    {right && <div className="m-2 h-auto max-w-lg"> <LazyLoadImage effect="blur" src={"https://res.cloudinary.com/infinityapis/image/upload"+img} alt="" placeholderSrc={place} className="object-cover block h-auto"/> </div>}
      ...
      {left && <div className="m-2 h-auto max-w-lg"> <LazyLoadImage effect="blur" placeholderSrc={place} src={img} alt="" className="object-cover"/> </div>}
    </div>
  );
};
