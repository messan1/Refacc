...
const SpeakerBox = ({right,left,img,content,titre,person}) => {
  return (
    ...
    {right && <div className="m-2 h-auto max-w-lg"> <LazyLoadImage effect="blur" src={img} alt="" className="object-cover block h-auto"/> </div>}
      ...
      {left && <div className="m-2 h-auto max-w-lg"> <LazyLoadImage effect="blur" src={img} alt="" className="object-cover"/> </div>}
    </div>
  );
};
